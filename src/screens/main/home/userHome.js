...
const UserHome = ({}) => {
  ...
  const getShopsNearYou = useCallback(() => {
    const onSuccess = (response) => {
      if (response.success) {
        const shops = response?.data?.shops || [];
        const groceries = response?.data?.groceries || [];

        dispatch(customerHomeActions.setGroceries(groceries));
        dispatch(customerHomeActions.setShopsNearMe(shops));
      }
    };

    const endpoint = `${API.shopsNearMe}/?latitude=${region?.latitude}&longitude=${region.longitude}&maxDistance=10&keyword=${debounceText}`;
    callApi(
      API_METHODS.GET,
      endpoint,
      null,
  ...
  }, [debounceText, dispatch, region?.latitude, region?.longitude]);
  ...
  return (
    <Screen>
      ...
      <Pressable
        onPress={() => locationBottomSheetRef.current.open()}
        style={homeStyles.header}
      >
        <View style={homeStyles.headerIconContainer}>
          <LocationFilledGrayIcon />
        </View>
        <AppText
          darkGrayText
      ...
      <View style={[homeStyles.searchContainer, globalStyles.screenPadding]}>
        ...
        <View style={homeStyles.iconsContainer}>
          ...
          <IconWrapper
            Icon={ChatGrayIcon}
            onPress={() => navigation.navigate(ROUTES.ChatInbox)}
          />
        </View>
      </View>
      ...
      <AppScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={false} onRefresh={handlePullToRefresh} />
        }
      >
        ...
        >
          ...
          <AppText
            fontSize={12}
            primary
            onPress={() => navigation.navigate(ROUTES.Groceries)}
          >
            See All
          </AppText>
        </View>
        ...
        <View style={{ flexGrow: groceriesLength > 0 ? undefined : 1 }}>
          <ScrollView
            ...
            nestedScrollEnabled
            horizontal
            contentContainerStyle={homeStyles.groceriesContentContainer}
            style={homeStyles.groceriesContainer}
          >
            {groceriesLength > 0 ? (
              groceries
                .slice(0, 5)
                .map((grocery, index) => (
                  <GroceryCard
                    ...
                    key={index}
                    item={grocery}
                    onPressPlusIcon={handleAddItemInCart}
                    onPress={handleGroceryItemPress}
                  />
                ))
            ) : (
        ...
        <View style={[homeStyles.sectionLabel, globalStyles.screenPadding]}>
          ...
          <AppText
            fontSize={12}
            primary
            onPress={() => navigation.navigate(ROUTES.Shops)}
          >
            See All
          </AppText>
        </View>
        ...
        <View style={{ flexGrow: shopsNearMeLength > 0 ? undefined : 1 }}>
          <ScrollView
            ...
            horizontal
            contentContainerStyle={homeStyles.groceriesContentContainer}
            style={homeStyles.shopsContainer}
          >
            {shopsNearMeLength > 0 ? (
              shopsNearMe
                .slice(0, 5)
                .map((shop, index) => (
                  <ShopCard
                    key={index}
                    item={shop}
                    style={homeStyles.shopCard}
                    onPress={handleShopPress}
                    onPressHeartIcon={handleLikeUnLikeShop}
                  />
                ))
            ) : (
              <View style={homeStyles.notfound}>
      ...
      <BottomSheet ref={locationBottomSheetRef} height={360}>
        <View style={homeStyles.bottomSheetContainer}>
          <View style={homeStyles.bottomSheetHeader}>
            <View style={homeStyles.bottomSheetIconContainer}>
              <LocationFilledGrayIcon />
            </View>
            <View style={homeStyles.bottomSheetTextContainer}>
              <AppText
                fontSize={16}
                fontFamily={FONTS.medium}
                style={homeStyles.bottomSheetTitle}
              >
                Search address
              </AppText>
              <AppText fontSize={12} style={homeStyles.bottomSheetSubtitle}>
                Pick a location to refresh nearby groceries and shops.
              </AppText>
            </View>
          </View>
          ...
          <GooglePlacesInput
            ...
            region={region}
            containerStyle={homeStyles.locationSearchContainer}
            listViewStyle={homeStyles.locationSearchList}
            textInputContainerStyle={homeStyles.locationSearchInputContainer}
            textInputStyle={homeStyles.locationSearchInput}
          />
        </View>
      </BottomSheet>
    </Screen>

      ...
      if (response.success) {
        ...
        const onSuccessPayment = async () => {
          ...
          if (verifyResult?.ok && verifyResult?.response?.success) {
            ...
            console.log(
              "[Checkout] payment verification success response",
          ...
          console.log(
            "[Checkout] payment verification failed response",
            verifyResult
          );
          onAPIError(
            verifyResult?.error ||
        ...
        const paymentSheetResult = await initializeAndPresentPaymentSheet(
          sheetData,
          onSuccessPayment
        );
        ...
        if (paymentSheetResult?.success && isPaymentVerified) {
          console.log(
            "[Checkout] cart reset dispatched after payment sheet closed and payment verified"
          );
        ...
        if (paymentSheetResult?.success) {
          console.log(
            "[Checkout] payment succeeded but verification did not complete, redirecting to My Cart tab"
          );
          // navigateToMyCart();
          navigateToHomeTab();
          return;
        ...
        if (paymentSheetResult?.isCanceled) {
          console.log(
            "[Checkout] payment sheet canceled by user, redirecting to My Cart tab"
          );
        } else {
          console.log(
            "[Checkout] payment flow ended with non-success state, redirecting to My Cart tab"
          );
        }
      ...
      <AppScrollView>
        ...
        <View style={[cartStyles.deliveryTime, globalStyles.flex1]}>
          <View style={{ gap: 4, marginBottom: 8 }}>
            <AppText fontFamily={FONTS.medium} fontSize={14}>
          ...
          <AppTextInput
            ...
            value={dayjs(deliveryTime).format("hh:mm A")}
          />
        </View>
